  ...
  node* create_node(Args&&... args)
  {
    ...
    try {
      std::allocator_traits<node_allocator>::construct(
          alloc_, new_node, std::forward<Args>(args)...);
      return new_node;
    } catch (...) {
...
public:
  // 构造函数
  list() : size_(0), alloc_() { init(); }

  explicit list(const Allocator& alloc) : size_(0), alloc_(alloc) { init(); }
  ...
  explicit list(size_type count, const Allocator& alloc = Allocator()) : size_(0), alloc_(alloc)
  {
    init();
    for (size_type i = 0; i < count; ++i) {
      emplace_back();
